...
export interface IThreadBuilder<Args extends any[]> {
	(count?: number | undefined): IThreadSpawner<Args>;
}

export interface IThreadSpawner<ArgType extends any[]> {
	spawn: (
...
export interface IThread<Args extends [...Args], Output> {
	/**
	 ...
	 */
	send<T extends Args = Args>(data: T): Promise<Output>;
